...
class cupones():
    ...
    def getInput(self):
        ...
        try:
            result.append(time.mktime(time.strptime('%s-%s-%s' %
                                                    (syear, smonth, sday), r'%Y-%m-%d')))
            result.append(time.mktime(time.strptime('%s-%s-%s' %
                                                    (fyear, fmonth, fday), r'%Y-%m-%d')))
        except:
            result = []
    ...
    def importCupones(self):
        ...
        with open('Cupones.csv', 'rt') as cf:
            ...
            for row in r:
                ...
                money = float(row['Importe'].replace(
                    '.', '').replace(',', '.'))
                self.db.execute(sql_insert_str, (d, row['Local'].lower(
                ), row['Tarjeta'].lower(), row['Lote'], money))
            cf.close()
            self.db.commit()
