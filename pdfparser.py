...
class pdfParser():
  ...
  def _parseVisa(self):
    ...
    filas = regex.findall(r'FECHA DE PAGO[\s\S]*?Total del d.a (?:\$[\d\. ]*,\d{2}){3}', raw_text)
    ano = int(regex.findall(r'\d{2}/\d{2}/(\d{4})\d{15}', raw_text)[0])
        
    # OJO: Matchea credito y debito!
    # Devuelve lista de listas
    # re_lotes = regex.compile(r'Liq\. N. (\d{8}).*?Lote N. (\d{4}).*?\d{1,} Ventas? (?:(?:en\s*\d{1,} pagos?.*?)|(?:Tj. ?D.bito))\$([\d\. ]*,\d{2})')
    re_liqnos = regex.compile(r'Liq\. N. (\d{8})')
    re_lotes = regex.compile(r'Lote N. (\d{4})')
    re_importes = regex.compile(r'\d{1,} Ventas? (?:(?:en\s*\d{1,} pagos?.*?)|(?:Tj. ?D.bito))\$([\d\. ]*,\d{2})')
    ...
    for fila in filas:
      fpago = dt.datetime.strptime(re_fpago.findall(fila)[0] + '/' + str(ano), '%d/%m/%Y').timestamp()
      arancel = self._convertMoneyToFloat(re_arancel.findall(fila)[0])
      impuestos = self._convertMoneyToFloat(re_impuestos.findall(fila)[0])
      ...
      try:
        financieros = self._convertMoneyToFloat(re_financieros.findall(fila)[0])
      except:
        financieros = 0.0
      
      try:
        adicionales = self._convertMoneyToFloat(re_adicionales.findall(fila)[0])
      except:
        adicionales = 0.0
      ...
      while (i < n):
        ...
        self.op['adicionales'] = (lapos + financieros + adicionales) / n
  ...
  def _parseMaestro(self):
    ...
    filas = regex.findall(r'Venta ctdo.*?F. Pres\d{2}/\d{2}/\d{4}', raw_text)

    # Matchea lote e importe
    re_ops = regex.compile(r'Venta ctdo\d{2}/\d{2}/\d{2}\d{6}(\d{3})\d{8}([\d\.]*,\d{2})')
    re_arancel = regex.compile(r'ARANCEL-\$([\d\.]*,\d{2})')
